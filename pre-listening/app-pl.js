    ...
    return `
      <div class="bg-white rounded-xl border ${isDone ? 'border-emerald-300 ring-1 ring-emerald-200' : 'border-slate-200'} p-4 shadow-sm hover:shadow-md transition flex flex-col justify-between">
        ...
        <div class="pt-3 border-t border-slate-100 flex items-center gap-2">
          ...
          <button onclick="window.openSubmitModalPL('${item.id}', '${item.title.replace(/'/g, "\\'")}')" class="flex-1 py-2 px-3 rounded-lg ${isDone ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-cyan-600 hover:bg-cyan-700'} text-white text-xs font-bold shadow transition flex items-center justify-center gap-1">
